...
public class CarBrandService (IUnitOfWork unitOfWork, IMapper mapper) : ICarBrandService
{
    ...
    public async Task<CarBrandResponse> CreateCarBrand(CarBrandRequest request)
    {
    ...
    public async Task<CarBrandResponse> UpdateCarBrand(Guid guid, CarBrandRequest request)
    {
        var carBrand = await unitOfWork.Repository<CarBrands>().GetByIdAsync(guid);
        if (carBrand == null)
        {
        ...
        await unitOfWork.Repository<CarBrands>().UpdateAsync(carBrand);
        await unitOfWork.SaveAsync();
        
        return mapper.Map<CarBrandResponse>(carBrand);
    }

    public async Task<bool> DeleteCarBrand(Guid guid)
    {
        var carBrand = await unitOfWork.Repository<CarBrands>().GetByIdAsync(guid);
        if (carBrand == null)
        {
